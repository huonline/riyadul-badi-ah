...
// ==========================================
// 3. DUMMY DATA UNTUK TEST TAMPILAN
// ==========================================
const dummyKitab = [
  {
    id: 1,
  ...
  },
  {
    id: 3,
    noBab: "٠٣",
    judulBab: "Fasl Arkanul Iman",
    deskripsi: "Rukun-rukun Iman",
    jumlahKata: 18,
    kataList: [
      { arab: "أَرْكَانُ", lugot: "ari rukun-rukunna" },
      { arab: "الإِيْمَانِ", lugot: "iman" },
      { arab: "سِتَّةٌ", lugot: "eta aya genep" },
    ]
  }
];
        ...
        <div className="max-w-2xl mx-auto p-4 md:p-6 pb-20">
          ...
          <header className="bg-amber-900 text-amber-50 rounded-2xl p-6 mb-6 shadow-md relative overflow-hidden">
            <div className="relative z-10">
              <span className="text-xs uppercase tracking-widest text-amber-300 font-semibold">Kitab Kuning Digital</span>
              ...
              <button 
                ...
                className="mt-5 px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-stone-950 font-semibold rounded-xl text-sm shadow transition flex items-center gap-2"
              >
                Mulai Membaca Bab 1
              </button>
            </div>
        ...
        <div className="min-h-screen flex flex-col justify-between max-w-3xl mx-auto p-4 md:p-6">
          ...
          <header className="sticky top-2 bg-white/90 backdrop-blur-md p-3 rounded-2xl border border-stone-200 shadow-sm z-20 flex flex-wrap justify-between items-center gap-2 mb-6">
            <button
              ...
              className="text-xs font-semibold px-3 py-1.5 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-lg transition"
            >
              ← Kembali
            </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-md border font-serif transition ${
                  showHarakat 
              ...
              {/* Toggle Lugot (Hide / Latin / Pegon) */}
              <select
                value={lugotMode}
                onChange={(e) => setLugotMode(e.target.value)}
                className="text-xs bg-white border border-stone-300 rounded-md px-2 py-1 focus:outline-none"
              >
          ...
          {/* Area Lembaran Kitab */}
          <main className="bg-[#fffdf9] p-6 md:p-10 rounded-2xl border border-amber-900/10 shadow-sm flex-1 mb-6 flex flex-col justify-between">
            ...
            {/* Loop Teks Matan + Lugot Miring */}
            <div className="flex flex-wrap flex-row-reverse items-end justify-start gap-x-6 gap-y-14 py-4" dir="rtl">
              {selectedBab.kataList.map((item, index) => {
                
                // Tentukan Teks Lugot berdasarkan Mode
                let displayLugot = '';
                if (lugotMode === 'latin') displayLugot = item.lugot;
                if (lugotMode === 'pegon') displayLugot = latinToPegon(item.lugot);

                return (
                  <div key={index} className="relative flex flex-col items-center min-w-[65px]">
                    
                    {/* Matan Arab (Berharakat / Gundul) */}
                    <span className="text-3xl font-serif text-stone-950 leading-none">
                      {showHarakat ? item.arab : removeHarakat(item.arab)}
                    </span>

                    {/* Lugot Sunda Miring 35 Derajat */}
                    {lugotMode !== 'hide' && (
                      <div
                      ...
                      >
                        <span 
                          className="text-[11px] text-amber-900 font-medium whitespace-nowrap bg-[#fffdf9]/90 px-1 rounded font-serif border border-amber-900/10 shadow-2xs"
